...
namespace DataBridge.Services
{
    ...
    public class WebServiceInfo
    {
        ...
        private WebMethodInfoCollection GetWebServiceDescription(string url, string portTypeName = "")
        {
            ...
            var serviceDescription = this.LoadServiceDescription(url);
            ...
            {
                var fallBackUrl = this.GenerateFallBackServiceUrl(url);
        ...
        private WebMethodInfoCollection ParseServiceDescription(ServiceDescription serviceDescription, string portTypeName = "")
        {
            ...
            foreach (PortType portType in serviceDescription.PortTypes)
            {
                ...
                foreach (Operation operation in portType.Operations)
                {
                    ...
                    var inputMessageName = operation.Messages.Input != null ? operation.Messages.Input.Message.Name : "";
                    var inputMessagePartName = !string.IsNullOrEmpty(inputMessageName)
                    ...
                    var outputMessageName = operation.Messages.Output != null ? operation.Messages.Output.Message.Name : "";
                    var outputMessagePartName = !string.IsNullOrEmpty(outputMessageName)
                    ...
                    var action = this.GetAction(serviceDescription, operation.Name);
            ...
            {
                var importedDescription = this.LoadServiceDescription(import.Location);
            ...
            foreach (Service service in serviceDescription.Services)
            {
                foreach (Port port in service.Ports)
                {
                    ...
                    {
                        var serviceUrl = (port.Extensions[0] as SoapAddressBinding).Location;
        ...
        /// <param name="messagePartName">Name of the message part.</param>
        /// <returns></returns>
        private List<Parameter> GetParameters(ServiceDescription serviceDescription, string messagePartName)
        {
            ...
            {
                var completeXmlSchema = this.ReadXmlSchemaIncludes(xmlSchema);
        ...
        private XmlSchema LoadSchema(string schemaLocation)
        {
            ...
            using (var reader = XmlReader.Create(schemaLocation, settings))
            {
        ...
        {
            var schemaAsString = String.Empty;
