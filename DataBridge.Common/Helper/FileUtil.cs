...
namespace DataBridge.Common.Helper
{
    ...
    {
        public static string SanitizeFileName(string fileName, string replacement = null)
        {
        ...
        public static void DeleteFile(string fileName, bool deletePermanent = false, bool forceDelete = true)
        {
            if (forceDelete)
            {
        ...
        public static Dictionary<string, string> RenameFiles(string searchDirectory, string searchString, string replaceString, string searchpattern = "*.*", string newPrefix = "", string newSuffix = "")
        {
            var renamedFiles = new Dictionary<string, string>();
            var fileInfos = new DirectoryInfo(searchDirectory).GetFiles(searchpattern);
            ...
            foreach (var fileInfo in fileInfos)
            {
                ...
                var newFileName = newPrefix +
                                  (!string.IsNullOrEmpty(searchString) ? fileNameNoExt.Replace(searchString, replaceString) : fileNameNoExt) +
                                  newSuffix +
                                  Path.GetExtension(fileInfo.Name);

                if (!Equals(oldFileName, newFileName))
                {
                    File.Move(searchDirectory + oldFileName, searchDirectory + newFileName);
                    renamedFiles.Add(oldFileName, newFileName);
                }
            }
