...
namespace DataBridge.Services
{
    ...
    public class SimpleWebserviceRequest
    {
        ...
        public SimpleWebserviceRequest(WebServiceInfo webServiceInfo, string methodName)
        {
            ...
            foreach (Parameter inputParam in methodInfo.InputParameters)
            {
        ...
        public string SoapAction
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(this.soapAction))
                {
                    ...
                    string ns = (string.IsNullOrEmpty(this.@namespace) ? "http://tempuri.org/" : this.@namespace);
                    if (!ns.EndsWith("/"))
                    {
                        ns += "/";
                    }
        ...
        public XmlDocument Invoke()
        {
            ...
            string requestXml = this.BuildRequestXml();
            ...
            try
            {
                ...
                byte[] requestBytes = this.encoding.GetBytes(requestXml);
                ...
                using (HttpWebResponse response = this.GetResponseWithRetry(request))
                {
        ...
        private XmlDocument ExtractResponseXml(Stream responseStream, string requestXml, bool hasError = false)
        {
            ...
            if (responseStream != null && responseStream.CanRead)
            {
                ...
                var responseNode = this.ExtractSoapResponse(xmlDoc);
                if (responseNode != null)
                {
                    ...
                    {
                        // Error Handling
                        string errorMsg = this.ExtractErrorMessage(responseDoc, requestXml);
        ...
        private XmlDocument ExtractSoapResult(XmlDocument responseDoc)
        {
            if (responseDoc == null)
            {
                return null;
            ...
            string innerXml = "";
            var resultNode = responseDoc.DocumentElement.FirstChild;
            ...
            {
                // autodetect escaping
                if (this.IsEscapedXml(resultNode.InnerXml) && !this.StartsWithXmlDeclaration(resultNode.InnerXml))
                {
                    innerXml = XmlHelper.UnescapeXml(innerXml);
                }
            }
        ...
        private string ExtractErrorMessage(XmlDocument responseDoc, string requestXml)
        {
            ...
            string errorMsg = responseDoc.DocumentElement.InnerText;
            if (!string.IsNullOrEmpty(errorMsg))
            {
        ...
        private string BuildRequestXml()
        {
            ...
            string requestXml = this.BuildSoapEnvelope();
            return requestXml;
        }
