...
namespace DataBridge.Services
{
    public class HttpServer
    {
        ...
        //public int[] httpPorts = new int[] { 8080, 5050 };
        //public int[] httpsPorts = new int[] { 4443 };
        ...
        private Action<HttpListenerRequest, HttpListenerResponse> requestHandlerAction;
        ...
        private void Listen()
        {
            ...
            string tempPrefix = "http://*:" + this.port + "/";
            this.listener.Prefixes.Add(tempPrefix);

            //foreach (var port in this.httpPorts)
            //{
            //    string temp_prefix = "http://*:" + port + "/";
            //    this.listener.Prefixes.Add(temp_prefix);
            //}

            //if (this.enable_https)
            //{
            //    foreach (var port in this.httpsPorts)
            //    {
            //        string temp_prefix = "https://*:" + port + "/";
            //        this.listener.Prefixes.Add(temp_prefix);
            //    }
            //}
        ...
        private void ListenerCallback(IAsyncResult result)
        {
            ...
            this.RequestHandlerAction(context.Request, context.Response);
        }

        public Action<HttpListenerRequest, HttpListenerResponse> RequestHandlerAction
        {
            get
            {
        ...
        private void RequestHandler(HttpListenerRequest request, HttpListenerResponse response)
        {
            ...
            if (File.Exists(docpath))
            {
                try
                {
                    ...
                    response.AddHeader("Last-Modified", System.IO.File.GetLastWriteTime(docpath).ToString("r"));
            ...
            else
            {
                if (this.debug)
                {
                    ...
                    responsestring += "root_path " + this.rootPath + " ";
                    responsebyte = Encoding.UTF8.GetBytes(responsestring);
                }
                else
                {
                    response.StatusCode = (int)HttpStatusCode.NotFound;
                    ...
                    {
                        responsebyte = File.ReadAllBytes(errorFile);
                    }
                    else
                    {
                        responsebyte = Encoding.UTF8.GetBytes("<H2>404 Error! File Does Not Exists...</H2>");
                    }
                }
            }
            response.ContentLength64 = responsebyte.Length;
